---
title: "GHG emissions"
format: html
---

::: {.grid}
::: {.g-col-6}

**Name:** GHG emissions

**Source:** [EDGAR](https://edgar.jrc.ec.europa.eu/dataset_ghg2025)

**Version:** EDGAR_2025_GHG

**Keywords:** climate change, mitigation, emissions, CO2, CH4, N2O

**Type:** Raster

**Description:** EDGAR_2025_GHG provides emissions of the three main greenhouse gases (CO2, CH4, N2O) and fluorinated gases per sector and country. Annual time series (1970-2024) and emission gridmaps (0.1 deg) are provided by sector, as well as monthly data for the last available year.

**Units:** NA

**Available in geocoded GMD?** NA

**Available from Space2Stats?** NA

:::
::: {.g-col-6}

**Map**

```{r}
#| include: false
map_path <- paste0("../code/", "NA",".R")
if (file.exists(map_path)){knitr::read_chunk(map_path)}
```

```{r}
#| label: NA
#| echo: false
#| message: false
#| warning: false
```
<small>
<small>
**Coordinate reference system:** NA

**Spatial coverage:** Global

**Spatial resolution:** NA

**Temporal coverage:** NA

**Temporal resolution:** NA

**License:** NA

**Citation:** Crippa, M., Guizzardi, D., Pagani, F., Banja, M., Muntean, M. et al., GHG emissions of all world countries - 2025 Report, Publications Office of the European Union, Luxembourg, 2025, doi:10.2760/9816914, JRC143227.
</small>
</small>

:::
:::

::::::: grid
::: {.g-col-lg-8 .g-col-lg-offset-2 .g-col-md-10 .g-col-md-offset-1 .g-col-sm-12 .g-col-12}


<small>
**Usage:** NA

**Limitations:** NA

**Examples:** NA

**Access:** NA
</small>

::: panel-tabset

## R
```{r}
#| echo: false
#| output: asis
if (file.exists("../code/NA")) {
  cat("```r
")
  try(cat(readLines("../code/NA"), sep = "
"))
  cat("```
")
} else {
  cat("No code
")
}
```

## Output
```{r}
#| echo: false
#| results: hold
if (file.exists("../code/NA")) {
  try(source("../code/NA"))
} else {
  print("None")
}
```

:::

**Last updated:** NA

:::
:::::::

```{=html}
<div class="secondary-links">
  <div class="secondary-links-container">
      <div class="secondary-links-title">
      </div>
      <div class="grid">
        <div class="g-col-lg-4 g-col-md-4 g-col-sm-12">
          <ul>
            <li><a href="http://GeoPov/">Geospatial Poverty Solutions Home</a></li>
          </ul>
        </div>
        <div class="g-col-lg-4 g-col-md-4 g-col-sm-12 center-left-border">
          <ul>
            <li><a href="https://pipmaps.worldbank.org/">Geospatial poverty portal</a></li>
          </ul>
        </div>
        <div class="g-col-lg-4 g-col-md-4 g-col-sm-12 center-left-border">
          <ul>
            <li><a href="mailto:bbrunckhorst@worldbank.org?subject=GeoPov%20data%20and%20methods%20repository" target="_top">Contact us</a></li>
          </ul>
        </div>
      </div>
    </div>
</div>
```


